...
本项目展示的是一个“LLM + 仿真模型 + 闭环控制可视化”的原型系统：

- LLM 不直接控制真实设备，而是在仿真闭环中输出结构化策略动作。
- 每次策略决策都基于当前状态，而不是一次性生成完整轨迹。
- 页面展示的是实时计算与控制过程，不是预先计算好的最终结果回放。
- 多策略对比用于观察不同控制逻辑在容量保持、Cycle 推进、温度和能量吞吐上的差异。
